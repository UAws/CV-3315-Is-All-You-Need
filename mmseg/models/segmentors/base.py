...
class BaseSegmentor(BaseModule, metaclass=ABCMeta):
    ...
    def show_result(self,
        ...
        Args:
            grey_out: to generate a grey-out image.
        ...
        if grey_out and out_file is not None:
            gray = np.uint8(seg)
            outlist = out_file.split('/')
            gray_path = f'{"/".join(outlist[:-1])}/gray/{outlist[-1]}'
            # print(gray_path)
            mmcv.imwrite(gray, gray_path)
